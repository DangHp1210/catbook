    ...
    <div class="cb-section-head">
        <div>
            <div class="cb-section-label">
                <span class="cb-section-tag cb-tag-bestseller">🏆 Bán chạy</span>
            </div>
            <h2 class="cb-section-title">Sách bán chạy nhất</h2>
        </div>
        <a href="{{ route('catalog.categories') }}?sort=best_seller" class="cb-see-all">Xem tất cả →</a>
    </div>
    ...
    <div class="cb-books-grid">
        @forelse($bestSellerBooks ?? [] as $book)
            @php
                ...
                $bgColors = ['#fef3c7','#fde8d8','#e8f5e9','#e0eaff','#fce4ec'];
                $txColors = ['#92400e','#7c3d12','#2e7d32','#1e3a8a','#880e4f'];
                $ci       = $loop->index % 5;
            ...
            <a href="{{ route('catalog.book', $book->slug) }}" class="cb-product-card">
                <div class="cb-product-img {{ $cover ? '' : 'no-img' }}" {!! $styleAttr !!}>
                    ...
                    @else
                        {{ $initial }}
                    @endif
                    <span class="cb-ribbon cb-ribbon-seller">#{{ $loop->iteration }} Bán chạy</span>
                </div>
        ...
        @empty
            <div class="cb-empty-state">Chưa có dữ liệu sách bán chạy.</div>
        @endforelse
    </div>
</section>
