...
.bd-cover:hover img { transform: scale(1.03); }
...
.bd-cover-ph-letter {
    ...
    font-size: 80px; font-weight: 900; color: #c5bdb0; line-height: 1;
}
.bd-cover-ph-label {
    font-size: 12px; color: #b0a898; font-family: var(--cb-sans);
}
...
.bd-cover-badge {
    ...
    padding: 3px 12px; border-radius: 999px; letter-spacing: .4px;
}

/* Meta chips under cover */
.bd-chips {
...
.bd-chip-lbl {
    ...
    text-transform: uppercase; color: #b0a898; margin-bottom: 3px;
}
.bd-chip-val { font-size: 13px; font-weight: 600; color: var(--cb-text); }
...
.bd-eyebrow {
    ...
    background: var(--cb-accent-light); padding: 4px 12px;
    border-radius: 999px; margin-bottom: 10px;
    font-family: var(--cb-sans);
}
.bd-title {
    font-family: var(--cb-serif);
    font-size: 34px; font-weight: 900; color: #0d1b10;
...
.bd-cat-tags { display: flex; flex-wrap: wrap; gap: 6px; }
.bd-cat-tag {
...
.bd-price-lbl {
    ...
    letter-spacing: 1.3px; text-transform: uppercase; color: #b0a898; margin-bottom: 8px;
}
.bd-price-row {
    display: flex; align-items: flex-end; gap: 12px; flex-wrap: wrap; margin-bottom: 12px;
}
.bd-price-main {
    font-family: var(--cb-serif); font-size: 38px; font-weight: 900;
...
.bd-stock-dot { width: 7px; height: 7px; border-radius: 50%; flex-shrink: 0; }
...
.bd-btn-cart:disabled { opacity: .45; cursor: not-allowed; }
...
.bd-btn-cart-link:hover { border-color: var(--cb-accent); color: var(--cb-accent); }
...
.bd-table-head {
    padding: 13px 20px; font-family: var(--cb-sans);
...
.bd-desc-empty {
    font-family: var(--cb-sans); font-size: 14px; color: #b0a898; font-style: italic;
}
/* read-more */
.bd-desc-inner { overflow: hidden; transition: max-height .3s ease; }
...
.bd-read-btn {
    margin-top: 10px; font-family: var(--cb-sans);
    font-size: 13px; font-weight: 600; color: var(--cb-accent);
...
.bd-reviews-hd {
    ...
    padding: 18px 22px; border-bottom: 1px solid var(--cb-border); flex-wrap: wrap;
}
.bd-reviews-title {
    font-family: var(--cb-serif); font-size: 20px; font-weight: 700; color: var(--cb-text);
}
.bd-rating-summary { display: flex; align-items: center; gap: 12px; }
.bd-rating-big {
...
.bd-av {
    ...
    display: flex; align-items: center; justify-content: center;
    flex-shrink: 0; font-family: var(--cb-sans);
}
.bd-rv-name {
...
@if(session('error'))
    <div class="bd-alert bd-alert-err">
        <svg width="15" height="15" fill="none" stroke="currentColor" stroke-width="2.2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg>
        {{ session('error') }}
    </div>
@endif
...
<div class="bd-grid">
    ...
    <article class="bd-right">
        ...
        <div>
            ...
            <p class="bd-authors">
                Tác giả:&nbsp;
                <strong style="color:var(--cb-text)">
                    {{ $book->authors->pluck('name')->join(', ') ?: 'Đang cập nhật' }}
                </strong>
            </p>

            @if($book->categories->isNotEmpty())
                <div class="bd-cat-tags">
                    @foreach($book->categories as $cat)
                        <a href="{{ route('catalog.category', $cat->slug ?? $cat->id) }}" class="bd-cat-tag">
                            {{ $cat->name }}
                        </a>
                    @endforeach
                </div>
        ...
        <div class="bd-price-card">
            ...
            <div class="bd-stock {{ $book->stock_quantity > 0 ? 'in' : 'out' }}">
                ...
                {{ $book->stock_quantity > 0
                    ? 'Còn hàng — '.$book->stock_quantity.' quyển'
                    : 'Tạm hết hàng' }}
            </div>
                ...
                @auth
                    <form method="POST"
                        ...
                        <div class="bd-qty-wrap">
                            <button type="button" class="bd-qty-btn"
                                    onclick="bdAdj(-1)">−</button>
                            <input id="bd-qty"
                                   type="number"
                                   name="quantity"
                                   class="bd-qty-input"
                    ...
                    <a href="{{ route('cart.index') }}" class="bd-btn-cart-link">
...
(function () {
    ...
    if (inner.scrollHeight <= 130) {
        inner.classList.remove('clamped'); btn.style.display = 'none'; return;
    }
    let open = false;
